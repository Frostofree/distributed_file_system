...
All functions to implement in chunk_server.py

1. stream_chunk:(fielname, offset, bytes)
	Reads the file from the offset and returns the bytes requested.
	- If the file does not exist, return 'failure'
	- If the offset is invalid, return 'failure'
	- If the bytes is invalid, return 'failure'
2. heartbeat:(chunk_id)
	- Send a heartbeat to the master to indicate that the chunk server is alive.
	- If the chunk server is dead, the master should be able to detect it.

3. wake_up(self)
	check for existing states from the logfile 
4. read_chunk(self, chunk_handle, offset, num_bytes)
	- Read the chunk from the offset and return the bytes requested.
	- If the chunk does not exist, return 'failure'
	- If the offset is invalid, return 'failure'
	- If the bytes is invalid, return 'failure'
5. write_and_yield_chunk(self, chunk_handle, loc_list, data_iterator)
	- Write the chunk and yield the data and further forwards data down the replication chain
6. write_chunk(self, chunk_handle, data_iterator)
	- Write the chunk and return the status
7. replicate_chunk(self, chunk_loc, chunk_handle)
	- Replicate the chunk to another chunk server
8. commit_chunk(self, chunk_handle)
	- Commit the chunk which means all the different chunk nodes have the chunk
9. delete_chunks(self, request_iterator)
	- Delete the chunks
10. chunk_cleanup(self)
	perodic cleanup of the chunks

ChunkToClientServicer:
	- create_chunk(self, request_iterator, context)
	- read_chunk(self, request, context)
CHunkToChunkServicer:
	- create_chunk(self, request_iterator, context)
	- read_entire_chunk(self, request, context)
ChunkToMasterServicer:
	- commit_chunk(self, request, context)
	- delete_chunks(self, request_iterator, context)
	- replicate_chunk(self, request, context)
	- heartbeat(self, request, context)
