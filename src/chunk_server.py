...
class ChunkServer():
    ...
    def create_chunk(self, client, chunk_handle, loc_list, iterator):
        ...
        if not loc_list:
            ...
            iterator = self.write_and_yield_chunk(
                chunk_handle, loc_list, iterator)
            nextChunk = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            nextChunk.connect((socket.gethostbyname(
                'localhost'), config.CHUNK_PORTS[loc_list[0]]))
    ...
    def replicate_chunk(self, client, chunk_loc, chunk_handle):
        nextChunk = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        nextChunk.connect((socket.gethostbyname('localhost'),
                          config.CHUNK_PORTS[chunk_loc]))
    ...
    def service(self, client, address):
        ...
        sender_type, command, args = parse_message(
